...
const defineDataRE = /(\/\*[\s\t]*defineData[\s\t]*\*\/[\s\t]*)([A-Za-z0-9]+([\s\t]*,[\s\t]*[A-Za-z0-9]+)*)([\s\n\t;])*/mg
const defineComponentRE = /(\/\*[\s\t]*defineComponent[\s\t]*\*\/[\s\t]*)([A-Za-z0-9]+([\s\t]*,[\s\t]*[A-Za-z0-9]+)*)([\s\n\t;])*/mg
const defineMethodRE = /(\/\*[\s\t]*defineMethod[\s\t]*\*\/[\s\t]*)([A-Za-z0-9]+([\s\t]*,[\s\t]*[A-Za-z0-9]+)*)([\s\n\t;])*/mg
...
export function createMarkdown(options: ResolvedOptions) {
  ...
  return (id: string, raw: string) => {
    ...
    const customDataBlocks: string[] = []
    const customComponentBlocks: string[] = []
    const customMethodBlocks: string[] = []
    if (isVue2) {
      hoistScripts.scripts = hoistScripts.scripts.map((line) => {
        line = line.replace(defineDataRE, (string, p1, p2) => {
          customDataBlocks.push(p2.replace(/[\s\t]+/g, ''))
          return string.replace(p1, '')
        })
        line = line.replace(defineComponentRE, (string, p1, p2) => {
          customComponentBlocks.push(p2.replace(/[\s\t]+/g, ''))
          return string.replace(p1, '')
        })
        line = line.replace(defineMethodRE, (string, p1, p2) => {
          customMethodBlocks.push(p2.replace(/[\s\t]+/g, ''))
          return string.replace(p1, '')
        })
        return line
      })
    ...
    const scripts = isVue2
      ? `<script>\n${scriptLines.join('\n')}\n${frontmatterExportsLines.join('\n')}\n${excerptExportsLine}export default { components: { ${customComponentBlocks.join(',')} },data() { return { frontmatter,${customDataBlocks.join(',')} } }, methods: { ${customMethodBlocks.join(',')} } }\n</script>`
